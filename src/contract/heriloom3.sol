 ...
 * @dev Manages digital asset inheritances with IPFS storage references and inheritance chains
 */
contract DigitalInheritance {
    ...
    function passDownInheritance(
        ...
        require(
            msg.sender == originalInheritance.owner || msg.sender == originalInheritance.successor,
            "Only owner or successor can pass down"
        );
        require(_newSuccessor != address(0), "Invalid successor address");
        require(_newSuccessor != msg.sender, "Cannot set yourself as successor");
        ...
        ipfsHashToInheritances[originalInheritance.ipfsHash].push(newInheritanceId);
     ...
     */
    function getInheritanceChain(uint256 _inheritanceId)
        external
        view
     ...
     */
    function getInheritanceChildren(uint256 _inheritanceId)
        external
        view
     ...
     */
    function getInheritancesByIpfsHash(string memory _ipfsHash)
        external
        view
        returns (uint256[] memory)
    {
     ...
     */
    function getRootInheritance(uint256 _inheritanceId)
        external
        view
        returns (uint256)
    {
     ...
     */
    function getOwnerInheritances(address _owner)
        external
        view
        returns (uint256[] memory)
    {
     ...
     */
    function getSuccessorInheritances(address _successor)
        external
        view
        returns (uint256[] memory)
    {
     ...
     */
    function canAccessInheritance(uint256 _inheritanceId, address _user)
        external
        view
        returns (bool)
    {
     ...
     * @param _owner Address of the owner
     */
    function getActiveInheritancesCount(address _owner)
        external
        view
        returns (uint256)
    {
